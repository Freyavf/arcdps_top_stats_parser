...
if __name__ == '__main__':
    ...
    for stat in config.stats_to_compute:
        ...
        if stat == 'barrier' or stat == 'deaths' or stat == 'kills':
            ...
            top_percentage_stat_players[stat],comparison_val = get_top_percentage_players(players, config, stat, num_used_fights, top_consistent_stat_players[stat], top_total_stat_players[stat])
        else:
            if stat == 'dist' or stat == 'dmg_taken':
                top_consistent_stat_players[stat] = get_top_players(players, config, stat, StatType.CONSISTENT)
                top_total_stat_players[stat] = get_top_players(players, config, stat, StatType.TOTAL)
                top_percentage_stat_players[stat],top_percentage_comparison_percentage[stat] = get_top_percentage_players(players, config, stat, num_used_fights, top_consistent_stat_players[stat])
            else:
                top_consistent_stat_players[stat] = get_top_players(players, config, stat, StatType.CONSISTENT)
                top_total_stat_players[stat] = get_top_players(players, config, stat, StatType.TOTAL)
                top_percentage_stat_players[stat],top_percentage_comparison_percentage[stat] = get_top_percentage_players(players, config, stat, num_used_fights, top_consistent_stat_players[stat], top_total_stat_players[stat])
